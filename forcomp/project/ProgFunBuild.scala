...
object ProgFunBuild extends Build {
  ...
  */
  ...
  val initGradingSetting = initGrading <<= (clean, sourceDirectory, baseDirectory) map { (_, submissionSrcDir, basedir) =>
  ...
  ere we depend on `initialize` because we already use the GradingFeedback
  ...
  setTestPropertiesHook = (test in Test) <<= (test in Test).dependsOn(setTestProperties)


  val scalaTest = TaskKey[Unit]("scalaTest")
  v
  /** *scalaTestSetting = scalaTest <<=
    (compile in Compile,
      compile in Test,
  ...
  al    * depend on compile to make sure the sources pass the compiler
    */
  ...
  */
  ...
  /**
  setMaxScore = TaskKey[Unit]("setMaxScore")
  v /***setMaxScoreSetting = setMaxScore <<= (gradeProjectDetails) map { project =>
    GradingFeedback.setMaxScore(project.maxScore, project.styleScoreRatio)
  }

  azyet the maximal score before running compile / test / ...
 ...
 def *********************************************************
  ...
  v/** getSubmissionSetting = getSubmission <<= (baseDirectory, scalaSource in Compile) map { (baseDir, scalaSrcDir) =>
    readAndUnpackSubmission(baseDir, scalaSrcDir)
  }

  // hependsOn makes sure that `getSubmission` is executed *before* `unmanagedSources`
  ...
  al readCompileLog = (compile in Compile) <<= (compile in Compile) mapR handleFailure(compileFailed)
  v// sreadTestCompileLog = (compile in Test) <<= (compile in Test) mapR handleFailure(compileTestFailed)
  ...
  al    * Only include the resource files which are defined in the package of the current project.
    */
  ...
  submission project takes test files from the main(assignment) project */
  val testSourcesFromAssignment = {
    (sourceDirectory in Test) <<= (sourceDirectory in(assignmentProject, Test))
  }

  // d   * Only include the test files which are defined in the package of the current project.
  * Also keeps test sources in packages listed in 'gradingTestPackages '
  */
  ...
  def scalaTestSubmission = TaskKey[Unit]("scalaTestSubmission")

  v
  /** *scalaTestSubmissionSetting = scalaTestSubmission <<=
    (compile in Compile,
     compile in Test,
     fullClasspath in Test,
     copyResources in Compile,
...
val *********************************************************
  ...
  al * -depend on scalaTestSubmission so that test get executed before style checking.the transitive
  ...
  */
  ...
  def apR: submit the grade / feedback in any

  case, also on failure
  val gradeSetting = grade <<= (gradingUUID, scalaTestSubmission, styleCheckSubmission, apiKey, gradeProjectDetails, streams) mapR { (uuidR, _, _, apiKeyR, projectDetailsR, s) =>
    ...
    apiKeyR match {
      case Value(apiKey) if (!apiKey.isEmpty) =>
        val apiKeyAugmented = apiKey + (projectDetails.courseId match {
          // OMG what a hack!!!
...
def failSubmit(): Nothing = {
  ...
  */
  ...
  val
deleteFiles(submissionSrcDir: File, basedir: File) {
  ...
  al
readAndUnpackSubmission(baseDir: File, targetSourceDir: File) {
    try {
    ...
    /***
failDownloadUnpack(): Nothing = {
    sys.error("Download or Unpack failed")
  }

  /**

handleFailure[R](handler: (Incomplete, String) => Unit) = (res: Result[R]) => res match {
    case Inc(inc) =>
    ...
    /***
compileFailed(inc: Incomplete, log: String) {
    GradingFeedback.compileFailed(log)
  }

  // m
compileTestFailed(inc: Incomplete, log: String) {
    GradingFeedback.testCompileFailed(log)
  }


 ca class ProjectDetails(packageName: String,
