...
import Settings._
import cc.spray.json.{DefaultJsonProtocol, JsNull, JsValue, JsonParser}
import dispatch.{Http, NoLogging, Request, StatusCode, url}
import org.apache.commons.codec.binary.{Base64, Hex}
import org.apache.commons.codec.digest.DigestUtils
import sbt._

import scalaz.Scalaz.{ValidationNEL, mkIdentity}
...
class Challenge(email: String, challengeKey: String, state: String)


se

class QueueResult(apiState: String)
...
object DeprectaionForwarder {
  ...
  eprecated("", "") def insecureClient(credentials: dispatch.Http.CurrentCredentials) = {
  ...
  @deprecated("", "") class FwdClass {
    def insecureClientForwarder(credentials: dispatch.Http.CurrentCredentials) = insecureClient(credentials)
  };
  ...
  cl
  ass NaiveTrustManager extends X509TrustManager {
  ...
  @dobject FwdClass extends FwdClass
}
...
caject CourseraHttp {
  ...
  * SUBMITTING
  ...
  /*
f submitSolution(sourcesJar: File, submitProject: ProjectDetails, challenge: Challenge, chResponse: String): ValidationNEL[String, String] = {
  ...
  de
f encodeBase64(bytes: Array[Byte]): String =
    new String(Base64.encodeBase64(bytes))

  de
f challengeResponse(challenge: Challenge, otPassword: String): String =
...
*******************************
  ...
  de
  f extractJson(jsonData: String, targetJar: File): ValidationNEL[String, QueueResult] =
  {
...
/*
f decodeBase64(str: String): Array[Byte] = {
  // codecs 1.4 has a version accepting a string, but not 1.2; jar hell.
  Base64.decodeBase64(str.getBytes)
}
}
/*
f unpackJar(file: File, targetDirectory: File): ValidationNEL[String, Unit] = {
  try {
...
de
ivate def executeRequest[T](req: Request)(parse: String => ValidationNEL[String, T]): ValidationNEL[String, T] = {
  try {
...
de
f fullExceptionString(e: Throwable) =
