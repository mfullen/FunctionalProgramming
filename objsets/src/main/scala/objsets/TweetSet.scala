...
class Empty extends TweetSet {
  ...
  def mostRetweetAcc(acc: Tweet) = acc
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  /**
   * If the Tweet elem matches the Boolean filter:
   * -Filter down the left and right side of the tree while including the original Matching tweet
   * else
   * -Filter down left side and right side of the tree without including the element
   */
  ...
  override def mostRetweetAcc(acc: Tweet): Tweet = {
    right.mostRetweetAcc(left.mostRetweetAcc(if (elem.retweets > acc.retweets) elem else acc))
  }
